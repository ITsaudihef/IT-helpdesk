          ...
          <div key={i} className="flex items-center flex-1">
            <div className={`flex items-center gap-2 ${i <= step ? "" : "opacity-40"}`}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all"
                style={{
                  background:  i < step ? "#6fb54a" : "#fff",
                  borderColor: i <= step ? "#6fb54a" : "#d1d5db",
                  color:       i < step ? "#fff" : i === step ? "#6fb54a" : "#9ca3af",
                }}>
                {i < step ? <Check className="w-4 h-4" /> : i + 1}
              </div>
              <span className="text-sm font-medium hidden sm:block"
                style={{ color: i <= step ? "#00805b" : "#9ca3af" }}>{s}</span>
            </div>
            {i < steps.length - 1 && (
              <div className="flex-1 h-0.5 mx-3 transition-all"
                style={{ background: i < step ? "#6fb54a" : "#e5e7eb" }} />
            )}
          </div>
        ))}
      </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {typeOptions.map((t) => (
                  <button key={t.value} type="button" onClick={() => setType(t.value)}
                    ...
                    style={{
                      borderColor: form.type === t.value ? "#6fb54a" : "#e5e7eb",
                      background:  form.type === t.value ? "#e0f1d0" : "#fff",
                    }}>
                    <div className="text-2xl mb-2">{t.icon}</div>
                    <div className="font-semibold text-sm" style={{ color: form.type === t.value ? "#00805b" : "#111827" }}>
                      {t.label}
                    </div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <div
              ...
              className="border-2 border-dashed rounded-xl p-10 text-center cursor-pointer transition-colors hover:bg-green-50/50"
              ...
              <ul className="space-y-2">
                {files.map((f, i) => (
                  <li key={i} className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 bg-gray-50">
                    <FileText className="w-5 h-5 flex-shrink-0" style={{ color: "#6fb54a" }} />
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: "#f4f4f5" }}>
              ...
              {files.length > 0 && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-gray-900">{files.length} ملف</span>
                </div>
              )}
