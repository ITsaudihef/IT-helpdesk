...
const typeOptions = [
  ...
  { value: "DEVELOPMENT",   label: "تطوير",           icon: "💻", desc: "طلبات تطوير أنظمة أو برمجيات" },
];
...
export default function CommSupportNewTicketPage() {
  ...
  const addFiles = (fl: FileList | null) => {
    ...
    Array.from(fl).forEach(f => {
      ...
      if (f.size > 2 * 1024 * 1024)   { toast.error(`الملف أكبر من 2MB: ${f.name}`);  return; }
      setFiles(p => [...p, f]);
    });
  };
    ...
    <div className="max-w-2xl mx-auto">
          ...
          <div key={i} className="flex items-center flex-1">
            <div className={`flex items-center gap-2 ${i <= step ? "" : "opacity-40"}`}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2"
                style={{ background: i < step ? "#7C3AED" : "#fff", borderColor: i <= step ? "#7C3AED" : "#d1d5db", color: i < step ? "#fff" : i === step ? "#7C3AED" : "#9ca3af" }}>
                {i < step ? <Check className="w-4 h-4" /> : i + 1}
              </div>
              <span className="text-sm font-medium hidden sm:block" style={{ color: i <= step ? "#5B21B6" : "#9ca3af" }}>{s}</span>
            </div>
            {i < steps.length - 1 && <div className="flex-1 h-0.5 mx-3" style={{ background: i < step ? "#7C3AED" : "#e5e7eb" }} />}
          </div>
        ))}
      </div>
      ...
      <div className="rounded-2xl p-6 shadow-sm" style={{ background: "#FFFFFF", border: "1px solid #E9E3FF" }}>
        ...
        <div className="flex gap-3 mt-6">
          ...
          {step < 2 ? (
            <button onClick={nextStep} className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white mr-auto" style={{ background: "#7C3AED" }}>
