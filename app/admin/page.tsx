...
export default async function AdminDashboard() {
  ...
  const kpis = [
    { label: "إجمالي التذاكر",   value: total,      icon: Ticket,        bg: "rgba(0,127,92,0.15)",  fg: "#007F5C", trend: "", href: "/admin/tickets" },
    ...
    { label: "بانتظار الاعتماد", value: pending,    icon: TrendingUp,    bg: "rgba(0,127,92,0.15)",  fg: "#007F5C", trend: pending > 0 ? "يحتاج انتباه!" : "", href: "/admin/tickets?status=PENDING_APPROVAL" },
    ...
    <div className="space-y-6">
      ...
      {pending > 0 && (
        <div className="rounded-xl p-4 flex items-center justify-between"
          ...
          <Link href="/admin/tickets?status=PENDING_APPROVAL"
            className="text-sm font-semibold hover:underline" style={{ color: "#A8DB98" }}>
            مراجعة الآن ←
          </Link>
        </div>
      )}
        ...
        {kpis.map((kpi) => {
          ...
          return (
            <Link key={kpi.label} href={kpi.href}
              ...
              <p className="text-2xl font-bold" style={{ color: "#16241D" }}>{kpi.value}</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold" style={{ color: "#16241D" }}>أحدث التذاكر</h2>
          <Link href="/admin/tickets" className="text-sm font-medium hover:underline" style={{ color: "#007F5C" }}>
            عرض الكل
          </Link>
        </div>
