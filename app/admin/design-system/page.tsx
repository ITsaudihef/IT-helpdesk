...
const NEUTRAL = [
  { name: "Page Background", hex: "#F3F7F1", usage: "خلفية الصفحة الأساسية" },
  { name: "Card Background", hex: "#FFFFFF", usage: "خلفية البطاقات والنوافذ الفاتحة" },
  { name: "Border",          hex: "#DCEAD9", usage: "حدود البطاقات وحقول الإدخال" },
  { name: "Text Primary",    hex: "#16241D", usage: "العناوين والنص الأساسي" },
  { name: "Text Secondary",  hex: "#00694A", usage: "نص ثانوي بلون العلامة" },
  { name: "Text Muted",      hex: "#5C7A6C", usage: "نص خافت / أوصاف ثانوية" },
];
...
function Swatch({ name, hex, usage, bg }: { name: string; hex: string; usage?: string; bg?: string }) {
  ...
  return (
    <button onClick={copy} className="text-right rounded-xl p-3 flex items-center gap-3 transition-all hover:shadow-md w-full"
      ...
      <div className="w-10 h-10 rounded-lg flex-shrink-0 border" style={{ background: bg ?? hex, borderColor: "#DCEAD9" }} />
      <div className="min-w-0 flex-1">
        <p className="text-sm font-semibold truncate" style={{ color: "#16241D" }}>{name}</p>
        {usage && <p className="text-xs mt-0.5 truncate" style={{ color: "#5C7A6C" }}>{usage}</p>}
      </div>
      <div className="flex items-center gap-1.5 flex-shrink-0">
        <span className="text-xs font-mono" style={{ color: "#007F5C" }}>{hex}</span>
...
function Section({ title, desc, children }: { title: string; desc?: string; children: React.ReactNode }) {
  return (
    <section className="mb-10">
      <h2 className="text-lg font-bold mb-1" style={{ color: "#16241D" }}>{title}</h2>
      {desc && <p className="text-sm mb-4" style={{ color: "#5C7A6C" }}>{desc}</p>}
      {children}
    </section>
  );
}
...
export default function DesignSystemPage() {
  return (
    <div dir="rtl">
      {/* Intro */}
      <div className="dark-modal rounded-2xl p-5 mb-8 flex items-center gap-4" style={{ background: "linear-gradient(135deg,#007F5C,#6FB449)" }}>
        ...
        <div>
          <h1 className="text-xl font-bold text-white">نظام التصميم — بوابة سند للدعم الفني</h1>
      ...
      <Section title="الخطوط" desc="خط واحد موحّد لكامل المنصة — Segoe UI">
        <div className="rounded-2xl p-6 space-y-3" style={{ background: "#FFFFFF", border: "1px solid #DCEAD9" }}>
          <p className="text-2xl font-bold" style={{ color: "#16241D" }}>عنوان رئيسي — Segoe UI Bold</p>
          <p className="text-lg font-semibold" style={{ color: "#16241D" }}>عنوان فرعي — Segoe UI Semibold</p>
          <p className="text-sm" style={{ color: "#16241D" }}>نص أساسي — يستخدم لكل محتوى الصفحات والنماذج</p>
          <p className="text-xs" style={{ color: "#5C7A6C" }}>نص ثانوي خافت — أوصاف وتلميحات</p>
        </div>
      </Section>
      ...
      <Section title="زوايا الاستدارة" desc="أربع قيم فقط، مستخدمة عبر Tailwind classes بشكل ثابت">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {RADIUS.map(r => (
            <div key={r.name} className={`p-4 text-center ${r.className}`} style={{ background: "#E3F2E0" }}>
              <p className="text-sm font-bold font-mono" style={{ color: "#007F5C" }}>{r.name}</p>
              <p className="text-xs mt-1" style={{ color: "#5C7A6C" }}>{r.usage}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section title="الأزرار" desc="زر أساسي واحد موحّد — لا تستخدم تدرجات بديلة لنفس الدور">
        <div className="rounded-2xl p-6 flex flex-wrap items-center gap-3" style={{ background: "#FFFFFF", border: "1px solid #DCEAD9" }}>
      ...
      <Section title="البطاقات والنوافذ">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: "1px solid #DCEAD9" }}>
            <p className="text-sm font-bold mb-1" style={{ color: "#16241D" }}>بطاقة فاتحة (Card)</p>
            <p className="text-xs" style={{ color: "#5C7A6C" }}>خلفية بيضاء + حدود بنفسجية فاتحة — الأكثر استخداماً في المنصة</p>
          </div>
          <div className="dark-modal rounded-2xl p-5" style={{ background: "#0F2A1F", border: "1px solid rgba(0,127,92,0.3)" }}>
            <p className="text-sm font-bold mb-1 text-white">نافذة داكنة (Dark Modal)</p>
            <p className="text-xs" style={{ color: "#8FCB7A" }}>
      ...
      <Section title="حقول الإدخال">
        <div className="rounded-2xl p-6 space-y-4 max-w-md" style={{ background: "#FFFFFF", border: "1px solid #DCEAD9" }}>
      ...
      {/* Guidance footer */}
      <div className="rounded-2xl p-5 text-sm" style={{ background: "#E3F2E0", color: "#00543D" }}>
