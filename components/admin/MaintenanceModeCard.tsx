...
export default function MaintenanceModeCard({ initialEnabled }: { initialEnabled: boolean }) {
  ...
  const toggle = async () => {
    const next = !enabled;
    if (next && !confirm("سيتم إيقاف وصول جميع المستخدمين (عدا مدراء النظام) عن بوابة الدعم لصندوق الوقف الصحي فوراً. هل تريد المتابعة؟")) return;
  ...
  return (
    <div className="rounded-2xl p-5" style={{ background: "#FFFFFF", border: `1px solid ${enabled ? "#FCA5A5" : "#DCEAD9"}` }}>
      <div className="flex items-center gap-2 mb-4">
        <div className="p-1.5 rounded-lg" style={{ background: "rgba(239,68,68,0.1)" }}>
          <Wrench className="w-4 h-4" style={{ color: "#DC2626" }} />
        </div>
        <h2 className="font-bold" style={{ color: "#16241D" }}>وضع الصيانة</h2>
      </div>

      <div className="flex items-center justify-between p-4 rounded-xl transition-all"
        style={{ background: enabled ? "rgba(239,68,68,0.08)" : "#F9F7FF", border: `1px solid ${enabled ? "#FCA5A5" : "#DCEAD9"}` }}>
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold" style={{ color: "#16241D" }}>
              {enabled ? "الصيانة مفعّلة الآن" : "إيقاف النظام للصيانة"}
            </p>
            <p className="text-xs mt-0.5" style={{ color: "#5C7A6C" }}>
